...
<body>
    ...
    <form action="AdminPaymentDetails.jsp" method="post">
...
<table align="center" border="1" width="1180px" style="border-collapse:collapse;">
    <tr style="background-color:yellowgreen;color:white;"><th><b>Serial No.</b></th><th><b>User ID</b></th><th><b>Payment ID</b></th><th><b>Game ID</b></th><th><b>Coach ID</b></th><th><b>Plan</b></th><th><b>Total Fee</b></th><th><b>Payment Date</b> </th>
		...
		try
		{
			...
			Connection connection=Helper.connect();
                    PreparedStatement ps=connection.prepareStatement("select payment.user_id,payment.payment_id,payment.game_id,member.coach_id,member.plan,payment.total_fee,payment.pay_date from member INNER JOIN payment ON member.game_id = payment.game_id where payment.user_id=member.id");
		...
		int j=1;
			try
			{
	            ...
	            Statement statement = con.createStatement(); 
	            ResultSet resultset =  statement.executeQuery("select payment.user_id,payment.payment_id,payment.game_id,member.coach_id,member.plan,payment.total_fee,payment.pay_date from member INNER JOIN payment ON member.game_id = payment.game_id where payment.user_id=member.id and user_id='"+cdate+"'") ; 
	            while(resultset.next())
	            {
	            	 out.print("<tr>");
                         out.print("<td>"+j+"</td>");
